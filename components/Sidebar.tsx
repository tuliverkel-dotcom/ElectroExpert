...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  return (
    <aside className="w-72 bg-slate-800 border-r border-slate-700 flex flex-col hidden lg:flex shadow-2xl z-10">
      ...
      <div className="p-4 space-y-2 bg-slate-800/50 border-b border-slate-700/50">
        <button 
          onClick={onNewProject}
          className="w-full bg-blue-600 hover:bg-blue-500 text-white text-[11px] font-black py-2.5 rounded-lg flex items-center justify-center gap-2 border border-blue-400 transition-all active:scale-95 shadow-lg shadow-blue-900/20"
        >
        ...
        <button 
          ...
          className="w-full bg-slate-900/50 hover:bg-slate-700/50 text-blue-400 text-[11px] font-black py-2.5 rounded-lg flex items-center justify-center gap-2 border border-blue-500/20 transition-all active:scale-95"
        >
        ...
        <button 
          ...
          className="w-full bg-slate-900/30 hover:bg-slate-700/30 text-slate-400 text-[11px] font-black py-2.5 rounded-lg flex items-center justify-center gap-2 border border-slate-700/50 transition-all active:scale-95 mt-2"
        >
      ...
      <div className="flex-1 overflow-y-auto px-4 space-y-6 py-4">
        ...
        <div>
          <h4 className="text-[9px] font-black text-green-500 uppercase tracking-tighter mb-3 flex items-center justify-between">
            Uložené riešenia
            <span className="bg-green-500/10 text-green-400 px-1.5 rounded">{savedProjects.length}</span>
          </h4>
          <div className="space-y-1.5">
            {savedProjects.length === 0 ? (
              <div className="text-[10px] text-slate-600 italic p-6 text-center border border-dashed border-slate-700 rounded-xl">
                Žiadne uložené riešenia
              </div>
            ) : (
              savedProjects.map((p) => (
                <div 
                  ...
                  className={`group flex items-center justify-between p-2.5 rounded-lg border transition-all cursor-pointer ${
                    ...
                    ? 'bg-green-600/10 border-green-500/50 text-green-400 shadow-lg shadow-green-900/10' 
                    : 'bg-slate-900/50 border-slate-700/50 text-slate-300 hover:border-slate-500 hover:bg-slate-900'
                  }`}
                >
                  <div className="flex flex-col overflow-hidden">
                    <span className="text-[10px] font-bold truncate">{p.name}</span>
                    <span className="text-[8px] opacity-50">{new Date(p.timestamp).toLocaleDateString()}</span>
                  </div>
                  <button onClick={(e) => onDeleteProject(p.id, e)} className="opacity-0 group-hover:opacity-100 text-slate-500 hover:text-red-500 transition-all p-1">
                    <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                  </button>
                </div>
              ))
